...
<br />
特朗普就职后，美国政府开放数据平台Data.gov的**近3000份数据集**被删除，总数从307,854降至305,564。数字档案社区发现，部分删除可能涉及**气候变化信息**等敏感内容，或与特朗普政府近期要求清理性别认同相关数据的行政令相关。尽管部分消失属行政交接的常规操作（如链接失效或数据迁移），但**环境科学机构**（能源部、NOAA、EPA）的数据删除比例异常突出。此前拜登就职时也有千份数据被移除，但区分“恶意清理”与行政疏失需逐一核查。
